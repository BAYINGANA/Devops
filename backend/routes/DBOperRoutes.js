...
router.get('/', async (req, res) => {
    try {
        const { rows } = await req.pool.query(`SELECT * FROM ${process.env.DB_TABLENAME}`);
...
router.post("/", async (req, res) => {
    ...
    try {
        ...
        // create the new user
        const insertResults = await req.pool.query(`INSERT INTO ${process.env.DB_TABLENAME} (name, email) VALUES ($1, $2) RETURNING id`, [name, email]);
...
router.put("/", async (req, res) => {
    ...
    try {
        ...
        //update the user
        await req.pool.query(`UPDATE ${process.env.DB_TABLENAME} SET name = $1, email = $2 WHERE id = $3`, [name, email, id])
